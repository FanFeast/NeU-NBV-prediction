...
def import_obj_dir(dir_path):
    ...
    with context.temp_override(**override):
        ...
        obj.location = [0, 0, obj.dimensions[-1]/2]
...
def setup_camera(num_frames, angle_space=45):
    ...
    for f in range(num_frames):
        rt = [
            (np.random.rand()-0.5) * angle_space,
            (np.random.rand()-0.5) * angle_space,
            0
        ]

        bpy.data.objects["camera_target"].rotation_euler = np.radians(rt)
        bpy.data.objects["camera_target"].keyframe_insert(data_path="rotation_euler", frame=f)

        bpy.data.objects["camera"].location[-1] = np.random.uniform(inner_radius, outer_radius)
        ...
        camera_matrices[f"RT_{f}"] = RT
        camera_matrices[f"K_{f}"] = K
...
if __name__ == "__main__":
    ...
    parser.add_argument('--obj_dir', type=str, help="Directory from which all obj files will be loaded and combined", default=None)
    parser.add_argument('--obj_dirs_file', type=str, help="file containing a list of obj directories", default=None)
    parser.add_argument('--frames', type=int, default=10,
                        help='How many frames to render')
    parser.add_argument('--angle_spaces', type=str, default="45",
                        help='comma seperated angle values')
    ...
    elif args.obj_dirs_file is not None:
        with open(args.obj_dirs_file) as f:
            obj_dirs = [x.strip() for x in f.readlines()]
    else:
        raise Exception("You must specify either an obj directory or multiple directories in an obj_paths_file")


    os.makedirs(args.out_dir, exist_ok=True)
    for i,obj_dir in enumerate(obj_dirs):
        obj = import_obj_dir(obj_dir)
        angles = [float(x) for x in args.angle_spaces.split(",")]
        for a in angles:
            p = os.path.join(args.out_dir, f"top{a}", f"scan{i}")
            os.makedirs(p, exist_ok=True)
            cams = setup_camera(args.frames, angle_space=a)
            np.savez(os.path.join(p, "cameras.npz"), **cams)
            render_scene_frames(os.path.join(p, "image"),
