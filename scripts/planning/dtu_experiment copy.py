...
class DTUNBVPlanning:
    ...
    def planning(self):
        ...
        selection_type = ["Ours"]
        # selection_type = ["M1", "M2"]


        # nview_list = [2, 3, 4, 5, 6, 7, 8, 9]  # maximal budget = 9
        nview_list = [2, 3, 4, 5, 6] #maximal budget = 5
        ...
        try:
            # JSON File
            json_filename = 'ref_index_record.json'
            with open('data.json', 'w') as fp:
                json.dump(ref_index_record, fp)
    ...
    def evaluation(self, index_record):
        ...
        with torch.no_grad():
            ...
            # for nviews, nviews_dict in tqdm(\\\index_record.items()):
                print(f"---------- {nviews} views experiment---------- \n")
                for scene_id, scene_dict in nviews_dict.items():
                    ...
                    for repeat, repeat_dict in scene_dict.items():
                        for stype, ref_index in repeat_dict.items():
                            ...
                            total_df=pandas.concat([total_df, dataframe], ignore_index=True)
